...
class LatentDiffusion_audio(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                ...
                xc = x
            if not self.cond_stage_trainable or force_c_encode:# caption
                if isinstance(xc, dict) or isinstance(xc, list):
                    c = self.get_learned_conditioning(xc)
                else:
    ...
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False,
        ...
        if cond is not None:
            if isinstance(cond, dict):
                cond = {key: cond[key][:batch_size] for key in cond}
            else:
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode") and self.cond_stage_key != "masked_image":
                ...
                if isinstance(caption,dict):
                    caption = caption['ori_caption'] + caption['struct_caption']
                xc = log_txt_as_img((256, 256), caption)
                log["conditioning"] = xc
